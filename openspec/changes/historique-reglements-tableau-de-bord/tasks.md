...
- [ ] 1.1 [T-126] Résoudre et vérifier T-126 (`resolve`/`verify`), créer/réutiliser
...
- [ ] 2.1 [T-126] Dans `features/dashboard/mocks/handlers.ts`, filtrer
...
- [ ] 2.2 [T-126] Ajouter au jeu de démonstration un règlement rattaché à une
...
- [ ] 3.1 [T-126] Dans `campaign-detail-page.ts`, lire un paramètre de route
...
- [ ] 3.2 [T-126] Dans `dashboard-page.html`/`.ts`, remplacer le `<span>`
      « Voir l'historique » par un `routerLink` vers `/campagnes/:id` avec ce
      paramètre, ciblant la campagne sélectionnée (`selectedCampaignId`).
- [ ] 3.3 [T-126] Masquer ce lien quand « Toutes les campagnes ouvertes » est
      sélectionné (aucune campagne précise à cibler).

## 4. Validation locale

- [ ] 4.1 [T-126] Ajouter/adapter les tests : `dashboard-page.spec.ts` (lien
...
- [ ] 4.2 [T-126] Exécuter `npm test -- --watch=false` et `npm run build`
