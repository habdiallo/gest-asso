...
function memberDetailPage() {
  ...
  const statusAction = isAdmin() ? (m.status === 'Actif'
    ? `<button class="btn btn-secondary" data-action="deactivate-member" data-success="Membre désactivé">${svg('lock')} Désactiver</button>`
    : `<button class="btn btn-secondary" data-action="reactivate-member" data-success="Membre réactivé">${svg('check')} Réactiver</button>`) : '';
  return `<section class="page"><div class="page-head"><div><button class="text-link" data-route="members">${svg('arrow')} Retour aux membres</button></div></div><div class="detail-layout"><div>
    <section class="hero-panel"><div class="hero-top"><div class="hero-identity"><div class="avatar">${m.initials}</div><div><h2>${m.name}</h2><div class="inline-meta">${status(m.status,m.status==='Actif'?'success':'neutral')}<span>Catégorie B</span><span>Président</span></div></div></div><div class="button-row">${edit}${statusAction}${isFinancialAllowed()?button('Enregistrer un règlement','payment-form','payments'):''}</div></div>
...
document.addEventListener('click', e => {
  ...
  const deactivateAction=e.target.closest('[data-action="deactivate-member"]');
  if(deactivateAction){members[0].status='Inactif';toast('Membre désactivé');render();return;}
  const reactivateAction=e.target.closest('[data-action="reactivate-member"]');
  if(reactivateAction){members[0].status='Actif';toast('Membre réactivé');render();return;}
